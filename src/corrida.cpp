...
void Corrida::criaObstaculo(int velObstaculo) {
	...
	if (true) {
		...
		obstaculosGiratorios.push_back(new ObstaculoGiratorio(velObstaculo));
		obstaculosGiratorios.back()->setTexture();
	}
	else { // if num == 2
		obstaculosVazados.push_back(new ObstaculoVazado(velObstaculo));
...
// Checa colis�es com obstaculos do tipo estatico.
void Corrida::colisaoEstatico() {
	...
	for (auto it = obstaculosEstaticos.begin(); it != obstaculosEstaticos.end(); ++it) {
		...
		for (auto it_player = player.begin(); it_player != player.end(); ++it_player) {
			// Se colidiu, ajusta a posi��o do boneco
...
// Checa colis�es com obstaculos do tipo giratorio.
...
// Checa colis�es com obstaculos do tipo vazado.
void Corrida::colisaoVazado() {			

	for (auto it = obstaculosVazados.begin(); it != obstaculosVazados.end(); ++it) {
		// Faz todas as checagens para cada player
		for (auto it_player = player.begin(); it_player != player.end(); ++it_player)
			// Se colidiu, verifica se n�o est� passando entre o(s) buraco(s)
			if (Collision::BoundingBoxTest((*it_player)->sprite, (*it)->sprite)) {
				...
				if ((*it)->loadedFile == "bin/obstaculo_3.png") { // Se for o obstaculo com apenas um buraco
					...
					(*it_player)->ajustaPosicao((*it)->sprite, (*it)->isVertical(), (*it)->getSpeed()); // Caso contrario, ajusta a posi��o do boneco
				}
...
// M�todo respons�vel por tratar os eventos de event
void Corrida::handleEvents(sf::Event& event, int& quit, bool& mostraFPS) {

	switch (event.type) {

		// Ao pressionar a tecla, o respectivo jogador come�ar� a subir
	case sf::Event::KeyPressed:
		if (event.key.code == sf::Keyboard::A) //Player 1
			player[0]->rise = true;
		if (event.key.code == sf::Keyboard::F) //Player 2
			player[1]->rise = true;
		if (event.key.code == sf::Keyboard::H && getNumPlayers() >= 3) //Player 3
			player[2]->rise = true;
	...
	case sf::Event::KeyReleased:
		if (event.key.code == sf::Keyboard::Escape) // Volta para o menu
			quit = 1;
		if (event.key.code == sf::Keyboard::F1) // Bot�o de toggle para o contador de FPS
			mostraFPS = !mostraFPS;
		...
		if (event.key.code == sf::Keyboard::F)
			player[1]->rise = false;
		if (event.key.code == sf::Keyboard::H && getNumPlayers() >= 3)
			player[2]->rise = false;
